...
#pragma warning restore 649
        ...
        private ConcurrentQueue<EntityAnalysisModelInvoke> pendingEntityInvoke;
        ...
        private DynamicEnvironment.DynamicEnvironment jubeEnvironment;
        ...
        {
            if (jubeEnvironment.AppSettings("EnableSanction").Equals("True",StringComparison.OrdinalIgnoreCase))
            {
        ...
        {
            if (jubeEnvironment.AppSettings("EnableExhaustiveTraining").Equals("True",StringComparison.OrdinalIgnoreCase))
            {
                log.Debug("Start: Starting Exhaustive Training Server.");

                var training = new Training(log, seeded, jubeEnvironment,contractResolver);
                trainingTask = Task.Run(training.Start);
        ...
        {
            if (jubeEnvironment.AppSettings("EnableCasesAutomation").Equals("True",StringComparison.OrdinalIgnoreCase))
            {
            ...
            try
            {
                ...
                while (!stopping)
                {
                    ...
                    if (notification != null)
                    {
                        ...
                        {
                            SendMail.Send(notification.NotificationDestination, notification.NotificationSubject, notification.NotificationBody, log,
                                jubeEnvironment);
                    ...
                    {
                        log.Debug("Notification Relay From Concurrent Queue: Nothing to relay.  Waiting for a second before trying again.");
        ...
        private void StartEntityModelServer()
        {
            ...
            if (jubeEnvironment.AppSettings("EnableEntityModel").Equals("True",StringComparison.OrdinalIgnoreCase))
            {
        ...
        {
            if (jubeEnvironment.AppSettings("ThreadPoolManualControl").Equals("True",StringComparison.OrdinalIgnoreCase))
            {
                ...
                while (!stopping)
                {
                    ...
                    if (tag != null)
                    {
                        try
                        {
                            ...
                            if (archive != null)
                            {
                                log.Info($"Tagging: Found record for {tag.EntityAnalysisModelInstanceEntryGuid} and model {tag.EntityAnalysisModelId} with id {archive.Id}. " +
                            ...
                            {
                                log.Info($"Tagging: No record for {tag.EntityAnalysisModelInstanceEntryGuid} and model {tag.EntityAnalysisModelId}.");
                            }
                        }
                        catch (Exception ex)
                        {
                            log.Info($"Tagging: Error on update for {tag.EntityAnalysisModelInstanceEntryGuid} and model {tag.EntityAnalysisModelId} as {ex}.");
                        }
                    }
                    else
                    {
        ...
        {
            if (lastCallbackTimeout.AddMilliseconds(100) < DateTime.Now)
            {
                ...
                try
                {
                    ...
                    log.Debug("Callback Timeout Management: Threshold for timeout is {threshold} and it has been offset from now by {callbackTimeout} ms.");
                    ...
                    foreach (var pendingCallback in EntityAnalysisModelManager.PendingCallbacks)
                    {
                        if (pendingCallback.Value.CreatedDate < threshold)
                        {
                            ...
                            log.Debug($"Callback Timeout Management: Expired callback {pendingCallback.Key} removed.  Counter incremented {PendingCallbacksTimeoutCounter}.");
                        } 
                    }
        ...
        {
            if (lastBalanceCountersWritten.AddSeconds(60) < DateTime.Now)
            {
