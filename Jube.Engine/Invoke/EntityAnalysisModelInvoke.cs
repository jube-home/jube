        ...
        Dictionary<int, EntityAnalysisModel> models)
    {
            ...
            ConcurrentQueue<EntityAnalysisModelInvoke> pendingEntityInvoke)
        {
            ...
            if (!InError)
            {
                ...
                if (async)
                {
                    ...
                    {
                        AsyncEnableCallback = jubeEnvironment.AppSettings("EnableCallback").Equals("True",StringComparison.OrdinalIgnoreCase);
        ...
        public void Start()
        {
            try
            {
                ...
                if (matchedGateway)
                {
                    pendingReadTasks.Add(ExecuteSanctions(pendingWriteTasks));
                    ExecuteDictionaryKvPs();
                    pendingReadTasks.Add(ExecuteTtlCounters());
                    ...
                    ExecuteCacheDbStorage(cachePayloadRepository, EntityAnalysisModelInstanceEntryPayloadStore,pendingWriteTasks);
                }
        ...
        private void QueueAsynchronousResponseMessage(List<Task> pendingWriteTasks)
        {
            ...
            if (AsyncEnableCallback) {
                ...
                pendingWriteTasks.Add(cacheCallbackRepository.Insert(ResponseJson.ToArray(),EntityAnalysisModelInstanceEntryPayloadStore.EntityAnalysisModelInstanceEntryGuid));
        ...
        private void ExecuteCacheDbStorage(CachePayloadRepository cachePayloadRepository,
            EntityAnalysisModelInstanceEntryPayload entityAnalysisModelInstanceEntryPayload,List<Task> pendingWriteTasks)
        {
            InsertOrReplaceCacheEntries(cachePayloadRepository, entityAnalysisModelInstanceEntryPayload,pendingWriteTasks);
        }
            ...
            EntityAnalysisModelInstanceEntryPayload entityAnalysisModelInstanceEntryPayload,List<Task> pendingWriteTasks)
        {
            if (EntityAnalysisModel.EnableCache)
            {
                if (Reprocess)
                {
                    pendingWriteTasks.Add(cachePayloadRepository.Upsert(entityAnalysisModelInstanceEntryPayload.TenantRegistryId,
                ...
                {
                    pendingWriteTasks.Add(cachePayloadRepository.Insert(entityAnalysisModelInstanceEntryPayload.EntityAnalysisModelId,
                        entityAnalysisModelInstanceEntryPayload.Payload, entityAnalysisModelInstanceEntryPayload.ReferenceDate,
                        entityAnalysisModelInstanceEntryPayload.EntityAnalysisModelInstanceEntryGuid));
        ...
        private void ActivationRuleTtlCounter(EntityAnalysisModelActivationRule evaluateActivationRule,List<Task> pendingWriteTasks)
        {
                            ...
                            if (CachePayloadDocumentStore.ContainsKey(foundTtlCounter.TtlCounterDataName))
                            {
                                ...
                                if (value.EnableTtlCounter)
                                {
                                        ...
                                        key)
                                    {
                                        ...
                                        {
                                            pendingWriteTasks.Add(cacheTtlCounterEntryRepository.Insert(
                                        ...
                                        pendingWriteTasks.Add(cacheTtlCounterRepository.Upsert(
                                            EntityAnalysisModel.Id,
                                            foundTtlCounter.TtlCounterDataName,
            ...
            if (evaluateActivationRule.SendToActivationWatcher && !suppressed && activationSample && !Reprocess &&
                ...
                try
                {
                        ...
                        EntityAnalysisModel.ActivationWatcherSample >= Seeded.NextDouble())
                    {
                        ...
                        if (jubeEnvironment.AppSettings("StreamingActivationWatcher").Equals("True",StringComparison.OrdinalIgnoreCase))
                        {
        ...
        private async Task ExecuteSanctions(List<Task> pendingWriteTasks)
        {
            log.Info(
                ...
                try
                {
                    if (CachePayloadDocumentStore.ContainsKey(entityAnalysisModelSanction.MultipartStringDataName))
                    {
                        ...
                        else
                        {
                            ...
                            var sanction = await cacheSanctionRepository.GetByMultiPartStringDistanceThreshold(
                            ...
                            else
                            {
                                ...
                                if (sanction == null)
                                {
                                    log.Info(
                                     ...
                                     pendingWriteTasks.Add(cacheSanctionRepository.Insert(EntityAnalysisModel.Id, multiPartStringValue,
                                ...
                                else
                                {
                                    ...
                                    pendingWriteTasks.Add(cacheSanctionRepository.Update(sanction.Id,averageLevenshteinDistance));
        ...
        private async Task ExecuteAbstractionRulesWithSearchKeys(CachePayloadRepository cachePayloadRepository)
        {
            ...
            if (EntityAnalysisModel.EnableCache)
            {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (CachePayloadDocumentStore.ContainsKey(key))
                            {
                                ...
                                pendingExecutionThreads.Add(execute.Start());
                            }
                            else
                            {
                                log.Info(
                ...
                var cacheAbstractionRepository = new CacheAbstractionRepository(jubeEnvironment.AppSettings("ConnectionString"),log);
                ...
                if (EntityAnalysisModel.EnableTtlCounter)
                {
                    ...
                    foreach (var ttlCounter in EntityAnalysisModel.ModelTtlCounters.FindAll(x => x.OnlineAggregation == false))
                    {
                        try
                        {
                            ...
                            var ttlCounterValue = await cacheTtlCounterRepository.GetByNameDataNameDataValue(
                                EntityAnalysisModel.Id,
                                ttlCounter.Id,
