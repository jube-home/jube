...
public class EntityAnalysisModelInvoke
{
    ...
    private readonly ConcurrentQueue<Notification> _pendingNotification;
    ...
    public Dictionary<string, Dictionary<string, double>> EntityInstanceEntryAdaptationResponses { get; set; } = new();
        ...
        ConcurrentQueue<EntityAnalysisModelInvoke> pendingEntityInvoke)
    {
                    ...
                    foreach (var xPath in entityAnalysisModel.EntityAnalysisModelRequestXPaths)
                    {
                        ...
                        if (!cachePayloadDocumentStore.ContainsKey(xPath.Name))
                        {
                            ...
                            try
                            {
                                ...
                                value = json.SelectToken(xPath.XPath);
                                ...
                                switch (xPath.DataTypeId)
                                {
                                    ...
                                    case 1:
                                    {
                                        if (value != null)
                                        {
                                            ...
                                            cachePayloadDocumentStore.Add(xPath.Name, valueString);
                                        ...
                                        {
                                            _log.Info(
                                                $"HTTP Handler Entity: GUID payload {entityInstanceEntryPayloadStore.EntityAnalysisModelInstanceEntryGuid} model id is {entityAnalysisModel.Id} {xPath.Name} XPath Path {xPath.XPath} has null and therefore no string will be added.");
                                        }
                                    ...
                                    case 2:
                                    {
                                        if (value != null)
                                        {
                                            ...
                                            cachePayloadDocumentStore.Add(xPath.Name, valueInt);
                                        ...
                                        {
                                            _log.Info(
                                                $"HTTP Handler Entity: GUID payload {entityInstanceEntryPayloadStore.EntityAnalysisModelInstanceEntryGuid} model id is {entityAnalysisModel.Id} {xPath.Name} XPath Path {xPath.XPath} has null and therefore no integer will be added.");
                                        }
                                    ...
                                    case 3:
                                    {
                                        if (value != null)
                                        {
                                            ...
                                            cachePayloadDocumentStore.Add(xPath.Name, valueDouble);
                                        ...
                                        {
                                            _log.Info(
                                                $"HTTP Handler Entity: GUID payload {entityInstanceEntryPayloadStore.EntityAnalysisModelInstanceEntryGuid} model id is {entityAnalysisModel.Id} {xPath.Name} XPath Path {xPath.XPath} has null and therefore no float will be added.");
                                        }
                                    ...
                                    //'Date Parse
                                    case 4 when DateTime.TryParse(value.ToString(), out var dateValue):
                                    {
                                        ...
                                        if (value != null)
                                        {
                                            switch (value.Type)
                                            {
                                                case JTokenType.Object:
                                                    _log.Info(
                                                        $"HTTP Handler Entity: GUID payload {entityInstanceEntryPayloadStore.EntityAnalysisModelInstanceEntryGuid} model id is {entityAnalysisModel.Id} {xPath.Name} XPath Path {xPath.XPath} has extracted value {valueBoolean} and been typed in the document as an Boolean.  It is of Object Type,  which implies an expression match.");
                                            
                                                    valueBoolean = true;
                                                    break;
                                                case JTokenType.Integer:
                                                    _log.Info(
                                                        $"HTTP Handler Entity: GUID payload {entityInstanceEntryPayloadStore.EntityAnalysisModelInstanceEntryGuid} model id is {entityAnalysisModel.Id} {xPath.Name} XPath Path {xPath.XPath} has extracted value {valueBoolean} and been typed in the document as an Boolean.  It has been parsed by JSON.net as an integer.  It will be evaluated for value to equal 1.");
                                            
                                                    valueBoolean = value.AsInt() == 1;
                                                    break;
                                                default:
                                                    _log.Info(
                                                        $"HTTP Handler Entity: GUID payload {entityInstanceEntryPayloadStore.EntityAnalysisModelInstanceEntryGuid} model id is {entityAnalysisModel.Id} {xPath.Name} XPath Path {xPath.XPath} has extracted value {valueBoolean} and been typed in the document as an Boolean.  It has been parsed by JSON.net as a string.  It will be evaluated for value to equal True.");
                                                
                                                    valueBoolean = value.AsString().Equals("True",
                                    ...
                                    case 6 or 7:
                                    {
                                        if (value != null)
                                        {
                                            ...
                                            cachePayloadDocumentStore.Add(xPath.Name, doubleValue);
                                        ...
                                        {
                                            _log.Info(
                                                $"HTTP Handler Entity: GUID payload {entityInstanceEntryPayloadStore.EntityAnalysisModelInstanceEntryGuid} model id is {entityAnalysisModel.Id} {xPath.Name} XPath Path {xPath.XPath} has null and therefore no geo position will be added.");
                                        }
                            ...
                            catch (Exception ex)
                            {
                                if (!cachePayloadDocumentStore.ContainsKey(xPath.Name))
                                {
                                    ...
                                    if (xPath.ResponsePayload)
                                    {
                                        cachePayloadDocumentResponse.Add(xPath.Name, valueString);
        ...
        if (!InError)
        {
            ...
            if (async)
            {
                ...
                {
                    AsyncEnableCallback = _jubeEnvironment.AppSettings("EnableCallback")
    ...
    private void QueueAsynchronousResponseMessage()
    {
        ...
        if (AsyncEnableCallback)
        {
            ...
            cacheCallbackRepository.Insert(ResponseJson.ToArray(),
        ...
        if (evaluateActivationRule.SendToActivationWatcher && !suppressed && activationSample && !Reprocess &&
            ...
            try
            {
                    ...
                    EntityAnalysisModel.ActivationWatcherSample >= Seeded.NextDouble())
                {
                    ...
                    if (_jubeEnvironment.AppSettings("ActivationWatcherAllowPersist")
                        .Equals("True", StringComparison.OrdinalIgnoreCase))
                    {
                    ...
                    if (_jubeEnvironment.AppSettings("StreamingActivationWatcher")
                        .Equals("True", StringComparison.OrdinalIgnoreCase))
                    {
    ...
    private void ExecuteAbstractionRulesWithSearchKeys(CachePayloadRepository cachePayloadRepository)
    {
        ...
        if (EntityAnalysisModel.EnableCache)
        {
            ...
            if (_jubeEnvironment.AppSettings("ForkAbstractionRuleSearchKeys")
                .Equals("True", StringComparison.OrdinalIgnoreCase))
            {
                _log.Info(
            ...
            var cacheAbstractionRepository =
                new CacheAbstractionRepository(_jubeEnvironment.AppSettings("ConnectionString"), _log);
                ...
                try
                {
                    if (abstractionRule.Search)
                    {
                                ...
                                x.Key == abstractionRule.SearchKey && x.Value.SearchKeyCache).Value != null)
                        {
                            ...
                            abstractionValue = cacheAbstractionRepository.GetByNameSearchNameSearchValueReturnValueOnly(
                                EntityAnalysisModel.Id,
                        ...
                        else
                        {
                            ...
                            abstractionValue = EntityAnalysisModelAbstractionRuleAggregator.Aggregate(
                                EntityAnalysisModelInstanceEntryPayloadStore,
