...
namespace Jube.App.Controllers.Repository
{
    ...
    public class EntityAnalysisAsynchronousQueueBalanceController : Controller
    {
        ...
        private readonly EntityAnalysisAsynchronousQueueBalanceRepository _repository;
        ...
        [HttpGet]
        public ActionResult<List<EntityAnalysisAsynchronousQueueBalanceDto>> Get()
        {
            try
            {
                if (!_permissionValidation.Validate(new[] {27})) return Forbid();

                return Ok(_mapper.Map<List<EntityAnalysisAsynchronousQueueBalanceDto>>(_repository.Get(60)));
            }
            catch (Exception e)
            {
                _log.Error(e);
