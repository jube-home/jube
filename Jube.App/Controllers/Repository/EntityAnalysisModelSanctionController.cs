...
namespace Jube.App.Controllers.Repository
{
    ...
    public class EntityAnalysisModelSanctionController : Controller
    {
        ...
        private readonly EntityAnalysisModelSanctionRepository _repository;
        private readonly string _userName;
        private readonly IValidator<EntityAnalysisModelSanctionDto> _validator;
            ...
            IHttpContextAccessor httpContextAccessor,DynamicEnvironment.DynamicEnvironment dynamicEnvironment)
        {
            ...
            _repository = new EntityAnalysisModelSanctionRepository(_dbContext, _userName);
            _validator = new EntityAnalysisModelSanctionDtoValidator();
        }
        ...
        public ActionResult<List<EntityAnalysisModelSanctionDto>> Get()
        {
            try
            {
                ...
                return Ok(_mapper.Map<List<EntityAnalysisModelSanctionDto>>(_repository.Get()));
            }
            catch (Exception e)
            {
                _log.Error(e);
        ...
        public ActionResult<List<EntityAnalysisModelSanctionDto>> GetByEntityAnalysisModelId(int entityAnalysisModelId)
        {
            try
            {
                ...
                return Ok(_mapper.Map<List<EntityAnalysisModelSanctionDto>>(
        ...
        public ActionResult<EntityAnalysisModelSanctionDto> GetById(int id)
        {
            try
            {
                ...
                return Ok(_mapper.Map<EntityAnalysisModelSanctionDto>(_repository.GetById(id)));
            }
            catch (Exception e)
            {
                _log.Error(e);
        ...
        public ActionResult<EntityAnalysisModelSanctionDto> Create([FromBody] EntityAnalysisModelSanctionDto model)
        {
            try
            {
                if (!_permissionValidation.Validate(new[] {11}, true)) return Forbid();

                var results = _validator.Validate(model);
                if (results.IsValid) return Ok(_repository.Insert(_mapper.Map<EntityAnalysisModelSanction>(model)));
        ...
        public ActionResult<EntityAnalysisModelRequestXPathDto> Update([FromBody] EntityAnalysisModelSanctionDto model)
        {
            try
            {
                ...
                var results = _validator.Validate(model);
                if (results.IsValid) return Ok(_repository.Update(_mapper.Map<EntityAnalysisModelSanction>(model)));
