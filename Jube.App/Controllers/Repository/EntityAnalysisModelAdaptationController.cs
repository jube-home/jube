...
namespace Jube.App.Controllers.Repository
{
    ...
    public class EntityAnalysisModelAdaptationController : Controller
    {
        ...
        private readonly EntityAnalysisModelHttpAdaptationRepository _repository;
        private readonly string _userName;
        private readonly IValidator<EntityAnalysisModelAdaptationDto> _validator;
            ...
            IHttpContextAccessor httpContextAccessor,DynamicEnvironment.DynamicEnvironment dynamicEnvironment)
        {
            ...
            _repository = new EntityAnalysisModelHttpAdaptationRepository(_dbContext, _userName);
            _validator = new EntityAnalysisModelAdaptationDtoValidator();
        }
        ...
        public ActionResult<List<EntityAnalysisModelAdaptationDto>> Get()
        {
            try
            {
                ...
                return Ok(_mapper.Map<List<EntityAnalysisModelAdaptationDto>>(_repository.Get()));
            }
            catch (Exception e)
            {
                _log.Error(e);
            ...
            int entityAnalysisModelId)
        {
            try
            {
                ...
                return Ok(_mapper.Map<List<EntityAnalysisModelAdaptationDto>>(
        ...
        public ActionResult<EntityAnalysisModelAdaptationDto> GetById(int id)
        {
            try
            {
                ...
                return Ok(_mapper.Map<EntityAnalysisModelAdaptationDto>(_repository.GetById(id)));
            }
            catch (Exception e)
            {
                _log.Error(e);
        ...
        public ActionResult<EntityAnalysisModelAdaptationDto> Create([FromBody] EntityAnalysisModelAdaptationDto model)
        {
            try
            {
                if (!_permissionValidation.Validate(new[] {15}, true)) return Forbid();

                var results = _validator.Validate(model);
                if (results.IsValid)
                    return Ok(_repository.Insert(
                        _mapper.Map<EntityAnalysisModelHttpAdaptation>(model)));
        ...
        public ActionResult<EntityAnalysisModelAdaptationDto> Update([FromBody] EntityAnalysisModelAdaptationDto model)
        {
            try
            {
                ...
                var results = _validator.Validate(model);
                if (results.IsValid)
                    return Ok(_repository.Update(_mapper.Map<EntityAnalysisModelHttpAdaptation>(model)));
